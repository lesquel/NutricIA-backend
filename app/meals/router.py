...
from app.meals.ai_provider import FoodAnalysisError
from app.meals.schemas import MealCreate, MealListResponse, MealResponse, ScanResult
...
async def scan_meal(file: UploadFile, user: CurrentUser) -> ScanResult:
    ...
    try:
        return await analyze_meal_image(image_bytes, file.content_type)
...
async def add_meal(body: MealCreate, user: CurrentUser, db: DB) -> MealResponse:
    ...
    meal = await create_meal(db, user.id, body)
...
async def list_meals(
    ...
    meals = await get_daily_meals(db, user.id, target_date)
...
async def get_meal(meal_id: str, user: CurrentUser, db: DB) -> MealResponse:
    """Get a single meal by ID."""
    meal = await get_meal_by_id(db, user.id, meal_id)
    if meal is None:
        raise HTTPException(
...
async def remove_meal(meal_id: str, user: CurrentUser, db: DB) -> None:
    """Delete a meal."""
    meal = await get_meal_by_id(db, user.id, meal_id)
    if meal is None:
        raise HTTPException(
